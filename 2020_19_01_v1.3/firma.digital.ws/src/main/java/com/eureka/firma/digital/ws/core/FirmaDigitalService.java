...
public class FirmaDigitalService implements IFirmaDigitalService {
	...
	public Resultado<Firma> generaFirma(	final PrivateKey llavePrivada,
		...
		try {		
			...
			firma.setFirmaElectronica( new String( Base64.encodeBase64( firmaDigital)));
	...
	public Resultado<PublicKey> validaCertificado(
		...
		try {
			fis = new ByteArrayInputStream( Base64.decodeBase64( certificadoBase64.getBytes()));
	...
	public Resultado<PrivateKey> validaLlavePrivada(
		...
		try {
			ByteArrayInputStream bais =  new ByteArrayInputStream( Base64.decodeBase64( llavePrivadaBase64.getBytes()));
		...
		try {
			...
			for ( File certiFile : directorioRootCerts.listFiles()) {
				...
				try {
                    ...
                    Log.info( "connecting to: " + urlServicioOCSP);
